...
- 保存、生成、上传、下载、重试、删除、返回、关闭、设置等具有明确动作语义的按钮，优先使用项目已安装的 `lucide-react` 对应图标配合中文动宾标签；禁止手绘 SVG、emoji 或 Unicode 字符冒充图标。图标不得替代文字，分段选择、是非确认或高密度行内操作在图标不能提升识别度时保持纯文字；仅通用且无歧义的控件允许纯图标，并必须提供可访问名称和 tooltip。
