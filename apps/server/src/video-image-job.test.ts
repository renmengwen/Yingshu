...
test("图片 Job 身份绑定完整批准版本、Prompt、模型和显式幂等键", () => {
  const permit: VideoImagePermit = {
    projectId: "project", videoId: "video", aspectRatio: "16:9",
    planSnapshotId: "snapshot", planSnapshotHash: "a".repeat(64),
  ...
  assert.notEqual(first, videoImageRequestIdentity({ ...permit, aspectRatio: "9:16" }, "provider", "model", "click-1"));
