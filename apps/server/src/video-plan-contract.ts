...
export function parseGeneratedScript(
  value: unknown,
  snapshot: FrozenVideoPlanSnapshot,
  sources: readonly VideoPlanSourceEvidence[] = [],
): VideoScriptContent {
  ...
  const sourceSummary = snapshot.input.webEnabled
    ...
    : [];
  return {
    title: planText(row.title, "标题建议", 100),
    summary: planText(row.summary, "内容摘要", 2_000),
    narration,
    estimatedCharacters,
    estimatedDurationSeconds: Math.round(estimatedCharacters / 3.5),
    paragraphs,
    sourceSummary,
    risks: optionalStringList(row.risks, "风险或待核对项"),
  };
}
...
export function parseVisualItems(
  value: unknown,
  snapshot: FrozenVideoPlanSnapshot,
  script: VideoScriptContent,
  editing: boolean,
): VideoVisualContent {
  ...
  const visuals = value.map((item, index) => {
    ...
    return {
      id,
      paragraphId,
      purpose: planText(row.purpose, "画面用途", 500),
      description,
      prompt: planText(row.prompt, "生图 Prompt", 8_000),
      negativePrompt: planText(row.negativePrompt, "负面 Prompt", 4_000),
      suggestedDurationSeconds: positiveNumber(row.suggestedDurationSeconds, "建议时长", 600),
      weight: positiveNumber(row.weight, "画面权重", 100),
...
export function scriptPrompt(snapshot: FrozenVideoPlanSnapshot, sources: readonly VideoPlanSourceEvidence[] = []) {
  ...
  const currentOperation = external?.usageRole === "topic_seed" || external?.usageRole === "content_source"
    ? { input: {
        ...
        visualDensity: creativeInput.visualDensity,
        aspectRatio: creativeInput.aspectRatio,
      },
      douyin,
      zhihu,
      webEnabled: creativeInput.webEnabled,
  ...
  return [
    "【固定系统合同】",
    "生成中文旁白方案。参考文本只有 referenceRole=content_source 时才可作为事实资料；style_only 仅参考表达方式。",
    "不得伪造人物、数字、引文、URL 或来源。短主题应扩写成目标时长量级的完整讲解，不重复凑字数。",
    ...
    "【全局补充】", snapshot.prompts.global.scriptInstructions || "（无）",
    "【项目补充】", snapshot.prompts.project.scriptInstructions || "（无）",
...
export function visualPrompt(snapshot: FrozenVideoPlanSnapshot, script: VideoScriptRevision) {
  ...
  return [
    "【固定系统合同】",
    `为已生成旁白规划 ${profile.aspectRatio}、${profile.width}×${profile.height} 的语义画面草案，不调用图片模型。图片内可读中文默认交给渲染层。`,
    ...
    "【全局补充】", snapshot.prompts.global.visualInstructions || "（无）",
    "【项目补充】", snapshot.prompts.project.visualInstructions || "（无）",
    "【视频补充】", snapshot.prompts.video.visualInstructions || "（无）",
    "【当前操作】", JSON.stringify({
      visualDensity: snapshot.input.visualDensity,
      targetDurationSeconds: snapshot.input.targetDurationSeconds,
      aspectRatio: snapshot.input.aspectRatio,
