...
import {
  ...
  type VideoPlanEntryMode,
} from "./types";

export function videoPlanLaunchBlockReason(
  input: VideoInputDraft,
  dirty: boolean,
  busy: boolean,
  modelAvailable: boolean,
  entryMode: VideoPlanEntryMode = "primary_input",
  entryBlockReason: string | null = null,
) {
  ...
  if (entryMode === "primary_input") {
    try {
      ...
      return error instanceof Error ? `${error.message}。` : "当前输入无效，请检查后重试。";
    }
  }
...
export function videoPlanLaunchConfirmation(input: VideoInputDraft, modelLabel: string) {
  const profile = getVideoOutputProfile(normalizeAspectRatio(input.aspectRatio));
  return {
    modelLabel,
    duration: `约 ${input.targetDurationSeconds} 秒`,
    web: input.webEnabled ? "已开启，将搜索并冻结可核验来源" : "已关闭，仅使用当前输入和参考资料",
    spec: `${profile.orientation} · ${profile.aspectRatio} · ${profile.width} × ${profile.height}`,
    steps: "资料准备 → 旁白 → 画面规划",
    boundary: "本次不会自动生成图片、配音、字幕或视频；方案生成后仍需人工审核。",
  };
}

export function VideoPlanLauncher({
  input,
  dirty,
  busy,
  modelLabel,
  modelAvailable,
  entryMode = "primary_input",
  ...
  const disabled = Boolean(
    videoPlanLaunchBlockReason(input, dirty, busy, modelAvailable, entryMode, entryBlockReason),
  );
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
          ...
          <AlertDialogDescription asChild>
            <div className="grid gap-4">
              ...
              <dl className="grid grid-cols-[auto_minmax(0,1fr)] gap-x-4 gap-y-2 text-[var(--fg-primary)]">
                <dt>文本模型</dt>
                <dd className="break-words font-medium">{confirmation.modelLabel}</dd>
                <dt>目标时长</dt>
                <dd>{confirmation.duration}</dd>
                <dt>输出规格</dt>
                <dd>{confirmation.spec}</dd>
                <dt>联网查证</dt>
                <dd>{confirmation.web}</dd>
                <dt>预计步骤</dt>
                <dd>{confirmation.steps}</dd>
              </dl>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            className="bg-[var(--accent)] text-[var(--accent-contrast)] hover:bg-[var(--accent-strong)]"
            onClick={onStart}
          >
            确认生成
          </AlertDialogAction>
