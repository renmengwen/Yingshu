...
import { currentImageCandidates, displayNarrationSummary, historicalImageCandidates, imageCandidateAspectRatio, type VideoImageVisual } from "./image-logic";
import { DEFAULT_PAGE_SIZE, PAGE_SIZE_OPTIONS, paginate } from "./video-plan-review/logic";
import { ImageLightbox, VisualImageReviewRow } from "./VisualImageReviewRow";
...
export function VideoImageReviewList({ visuals, productionAllowed, busyAction, onGenerate, onUpload, onApprove }: {
  ...
  const [lightboxVisualId, setLightboxVisualId] = useState<string | null>(null);
  const [lightboxCandidateId, setLightboxCandidateId] = useState<string | null>(null);
  ...
  const lightboxVisual = visuals.find((visual) => visual.id === lightboxVisualId) ?? null;
  ...
  function openImage(visualId: string, candidateId: string, trigger: HTMLElement) {
    lightboxTriggerRef.current = trigger;
    setLightboxCandidateId(candidateId);
    setLightboxVisualId(visualId);
  }
  ...
  return <section className="min-w-0 px-4 py-5 md:px-7" aria-labelledby="image-records-heading">
    ...
    <div className="hidden overflow-x-auto border-x border-b border-[var(--border-subtle)] md:block">
      <Table className="min-w-[900px] table-fixed"><TableCaption className="sr-only">配图候选与审核记录</TableCaption><TableHeader><TableRow><TableHead className="w-24">画面</TableHead><TableHead className="w-[34%]">旁白摘要</TableHead><TableHead className="w-32">生成状态</TableHead><TableHead className="w-20">候选</TableHead><TableHead className="w-28">当前批准图</TableHead><TableHead className="w-32">修订状态</TableHead><TableHead className="w-32 text-right">操作</TableHead></TableRow></TableHeader><TableBody>{view.items.map((visual) => {
        ...
        const summary = displayNarrationSummary(visual.narrationSummary);
        return <TableRow key={visual.id}><TableCell><div className="flex items-center gap-2"><button className="h-14 w-24 shrink-0 overflow-hidden bg-[var(--bg-inset)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)]" type="button" disabled={!thumbnail} aria-label={`查看画面 ${visualNumber} 大图`} onClick={(event) => thumbnail && openImage(visual.id, thumbnail.id, event.currentTarget)}>{thumbnail ? <img className="h-full w-full object-contain" style={{ aspectRatio: imageCandidateAspectRatio(thumbnail) }} src={thumbnail.previewUrl} alt={`画面 ${visualNumber} 当前候选缩略图`} /> : null}</button><span className="font-mono">{visualNumber}</span></div></TableCell><TableCell className="whitespace-normal break-words"><p className="line-clamp-2 break-words leading-5">{summary}</p><p className="mt-1 line-clamp-1 break-words text-xs text-[var(--fg-tertiary)]">{visual.description}</p></TableCell><TableCell className="whitespace-normal break-words"><Badge variant={visual.generationState?.status === "failed" ? "destructive" : "outline"}>{generationLabel(visual)}</Badge>{visual.generationState?.errorSummary ? <p className="mt-1 line-clamp-2 break-words text-xs text-[var(--status-danger)]">{visual.generationState.errorSummary}</p> : null}</TableCell><TableCell className="font-mono">{current.length}</TableCell><TableCell>{approved ? <button className="block h-14 w-24 overflow-hidden bg-[var(--bg-inset)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)]" type="button" aria-label={`查看画面 ${visualNumber} 当前批准图大图`} onClick={(event) => openImage(visual.id, approved.id, event.currentTarget)}><img className="h-full w-full object-contain" style={{ aspectRatio: imageCandidateAspectRatio(approved) }} src={approved.previewUrl} alt={`画面 ${visualNumber} 当前批准图缩略图`} /></button> : <span className="text-xs text-[var(--fg-secondary)]">未批准</span>}</TableCell><TableCell className="whitespace-normal break-words"><Badge variant="outline">当前修订</Badge>{historyCount ? <p className="mt-1 break-words text-xs text-[var(--status-warning)]">存在 {historyCount} 个历史已失效候选</p> : <p className="mt-1 break-words text-xs text-[var(--fg-secondary)]">无历史失效候选</p>}</TableCell><TableCell className="text-right"><Button aria-label={`画面 ${visualNumber}：${actionLabel}`} variant="outline" type="button" onClick={(event) => openDetails(visual.id, event.currentTarget)}>{actionLabel}</Button></TableCell></TableRow>;
      })}</TableBody></Table>
    </div>
    <div className="divide-y divide-[var(--border-subtle)] border-x border-b border-[var(--border-subtle)] md:hidden">{view.items.map((visual) => { const current = currentImageCandidates(visual); const approved = current.some((candidate) => candidate.approved); const visualNumber = String(visual.order + 1).padStart(2, "0"); const actionLabel = current.length ? approved ? "查看候选" : "选择与批准" : "查看与生成"; return <article key={visual.id} className="min-w-0 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-mono text-xs font-semibold text-[var(--accent)]">画面 {visualNumber}</p><p className="mt-2 line-clamp-2 text-sm leading-6">{displayNarrationSummary(visual.narrationSummary)}</p></div><Badge variant={approved ? "default" : "outline"}>{reviewLabel(visual)}</Badge></div><p className="mt-2 text-xs text-[var(--fg-secondary)]">{generationLabel(visual)} · {current.length} 个当前候选</p><Button aria-label={`画面 ${visualNumber}：${actionLabel}`} className="mt-3 w-full" variant="outline" type="button" onClick={(event) => openDetails(visual.id, event.currentTarget)}>{actionLabel}</Button></article>; })}</div>
    ...
    {lightboxVisual ? <ImageLightbox visual={lightboxVisual} candidate={[...currentImageCandidates(lightboxVisual), ...historicalImageCandidates(lightboxVisual)].find((candidate) => candidate.id === lightboxCandidateId) ?? currentImageCandidates(lightboxVisual)[0] ?? historicalImageCandidates(lightboxVisual)[0] ?? null} open triggerRef={lightboxTriggerRef} onOpenChange={(next) => { if (!next) { setLightboxVisualId(null); setLightboxCandidateId(null); } }} /> : null}
  </section>;
}
