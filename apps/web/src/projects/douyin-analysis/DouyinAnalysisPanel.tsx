...
import { BanIcon, EyeIcon, LoaderCircleIcon, PlayIcon, RefreshCwIcon } from "lucide-react";
import {
...
import type { useDouyinAnalysis } from "./use-douyin-analysis";
...
export function DouyinAnalysisPanel({ projectId, videoId, state }: {
  projectId: string;
  videoId: string;
  state: ReturnType<typeof useDouyinAnalysis>;
}) {
  ...
  const rows = evidenceRows(snapshot?.evidence ?? null, snapshot?.config ?? state.draft);
  const validationError = validateDouyinDraft(state.draft);
  const serverAllowsStart = state.summary?.allowedActions.start ?? false;
  const startBlockReason = !state.loaded ? "正在恢复抖音分析状态，请稍候。" : validationError
  ...
  return <section className="min-w-0" aria-labelledby="douyin-analysis-heading">
    <div className="flex min-w-0 flex-col gap-6">
      ...
      <p className={`text-sm ${state.error ? "font-medium text-[var(--danger)]" : "text-[var(--fg-secondary)]"}`} role={state.error ? "alert" : "status"} aria-live={state.error ? undefined : "polite"}>{state.status}</p>
      ...
      {snapshot && state.summary?.allowedActions.selectUsage ? <section className="grid min-w-0 gap-5 border-t border-[var(--border-subtle)] pt-6" aria-labelledby="douyin-usage-heading">
        <div><h4 id="douyin-usage-heading" className="text-sm font-semibold">选择使用方式</h4><p className="mt-2 text-sm leading-6 text-[var(--fg-secondary)]">修改后会自动保存。切换方式不会重跑下载、ASR 或抽帧，但会使现有文案、画面方案和下游失效。</p></div>
