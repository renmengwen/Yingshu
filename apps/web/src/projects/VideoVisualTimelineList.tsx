...
export function VideoVisualTimelineList({ segments, timelineHash, stale, issues, busyAction, onDirtyChange, onSave }: {
  ...
  return <section className="min-w-0 px-4 py-5 md:px-7" aria-labelledby="visual-segments-heading">
    ...
    {openSegment ? <VisualSegmentDialog segment={openSegment} open triggerRef={triggerRef} busyAction={busyAction} onDirtyChange={onDirtyChange} onOpenChange={(next) => { if (!next) setOpenSegmentId(null); }} onSave={(patch) => onSave(openSegment.id, patch)} /> : null}
  </section>;
}

function VisualSegmentDialog({ segment, open, triggerRef, busyAction, onDirtyChange, onOpenChange, onSave }: {
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!next) requestClose(); }}><DialogContent className="w-[min(calc(100vw-2rem),64rem)]" showCloseButton={false} onOpenAutoFocus={(event) => { event.preventDefault(); initialFocusRef.current?.focus(); }} onCloseAutoFocus={(event) => { event.preventDefault(); triggerRef.current?.focus(); }}>
    ...
    <div className="grid min-w-0 gap-5 p-4 sm:p-6 lg:grid-cols-[15rem_minmax(0,1fr)]">
      <figure><img src={segment.previewUrl} alt={`视觉段 ${number} 当前批准图片`} className="mx-auto aspect-[9/16] max-h-[52vh] w-full bg-[var(--bg-inset)] object-contain" /><figcaption className="mt-2 break-all font-mono text-[10px] text-[var(--fg-secondary)]">当前批准图 · {segment.candidateHash}</figcaption></figure>
