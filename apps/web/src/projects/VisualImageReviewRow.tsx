...
import { candidateIdentity, currentImageCandidates, formatImageCandidateTime, historicalImageCandidates, imageCandidateAspectRatio, styleSnapshotLabel, type VideoImageCandidate, type VideoImageVisual } from "./image-logic";
        ...
        <div className="grid min-w-0 gap-6 p-4 sm:p-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(18rem,.85fr)]">
          <div className="min-w-0 space-y-5">
            ...
            {current.length ? <section className="space-y-3"><div><h4 className="text-sm font-semibold">选择一个候选</h4><p className="mt-1 text-xs leading-5 text-[var(--fg-secondary)]">选择只保留在当前弹框；点击“批准所选图片”后才会写入审核结果。</p></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{current.map((candidate, index) => <article key={candidate.id} className={`min-w-0 border p-2 ${selectedId === candidate.id ? "border-[var(--accent)] bg-[var(--surface-selected)]" : "border-[var(--border-subtle)]"}`}>
              <button className="group relative block w-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)]" type="button" onClick={(event) => openLightbox(candidate, event.currentTarget)}><img className="w-full bg-[var(--bg-inset)] object-cover" style={{ aspectRatio: imageCandidateAspectRatio(candidate) }} src={candidate.previewUrl} alt={`候选 ${index + 1}，点击查看大图`} /><span className="pointer-events-none absolute inset-x-1 bottom-1 bg-black/65 px-1 py-1 text-center text-[10px] text-white opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100">查看大图</span></button>
            ...
            {historical.length ? <details className="border-t border-[var(--border-subtle)] pt-3"><summary className="min-h-11 cursor-pointer py-3 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)]">历史 / 已失效候选（{historical.length}）</summary><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{historical.map((candidate, index) => <button key={candidate.id} className="border border-[var(--border-subtle)] p-2 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)]" type="button" onClick={(event) => openLightbox(candidate, event.currentTarget)}><img className="w-full object-cover opacity-70" style={{ aspectRatio: imageCandidateAspectRatio(candidate) }} src={candidate.previewUrl} alt={`历史候选 ${index + 1}，点击查看大图`} /><span className="mt-2 block text-xs">历史候选 {index + 1}</span></button>)}</div></details> : null}
          </div>
          <div className="min-w-0 space-y-5">
