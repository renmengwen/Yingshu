...
type ProductionSettings = Pick<VideoInputDraft, "targetDurationSeconds" | "visualDensity" | "visualInstructions" | "webEnabled" | "scriptInstructions">;
...
export function VideoProductionSettingsDialog({ disabled, settings, onApply }: VideoProductionSettingsDialogProps) {
  ...
  return <>
    <Dialog open={open} onOpenChange={requestOpen}>
      <DialogTrigger asChild><Button type="button" variant="outline" disabled={disabled}><Settings2Icon aria-hidden="true" />编辑制作设置</Button></DialogTrigger>
        ...
        <div className="grid gap-6 p-5 sm:p-6">
          <div className="grid gap-6 sm:grid-cols-2">
            <FieldSet disabled={disabled}>
              ...
              <RadioGroup className="grid grid-cols-2 gap-2" name="dialog-duration" value={String(draft.targetDurationSeconds)} onValueChange={(value) => setDraft({ ...draft, targetDurationSeconds: Number(value) })}>
                {([[60, "1 分钟"], [180, "3 分钟"], [300, "5 分钟"], [600, "10 分钟"]] as const).map(([value, label]) => <FieldLabel className={segmentedOptionClass} key={value}><RadioGroupItem className={hiddenRadioClass} value={String(value)} /><span>{label}</span></FieldLabel>)}
              </RadioGroup>
            </FieldSet>
            <FieldSet disabled={disabled}>
              ...
              <RadioGroup className="grid grid-cols-3 gap-2" name="dialog-visual-density" value={draft.visualDensity} onValueChange={(value) => setDraft({ ...draft, visualDensity: value as VideoInputDraft["visualDensity"] })}>
                {([['relaxed', '舒缓'], ['standard', '标准'], ['compact', '紧凑']] as const).map(([value, label]) => <FieldLabel className={segmentedOptionClass} key={value}><RadioGroupItem className={hiddenRadioClass} value={value} /><span>{label}</span></FieldLabel>)}
              </RadioGroup>
          ...
          <FieldSet disabled={disabled}>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4" role="group" aria-describedby="dialog-visual-style-help" aria-label="生图画风预设">
              {VISUAL_STYLE_PRESETS.map((preset) => <Button key={preset.id} type="button" variant="outline" className={`${segmentedOptionClass} h-auto whitespace-normal py-2`} aria-pressed={visualStylePreset === preset.id} data-state={visualStylePreset === preset.id ? "checked" : "unchecked"} onClick={() => setDraft({ ...draft, visualInstructions: applyVisualStylePreset(draft.visualInstructions, preset.id) })}>{preset.label}</Button>)}
              <Button type="button" variant="outline" className={`${segmentedOptionClass} h-auto whitespace-normal py-2 sm:col-span-2`} aria-pressed={visualStylePreset === null} data-state={visualStylePreset === null ? "checked" : "unchecked"} onClick={() => setDraft({ ...draft, visualInstructions: applyVisualStylePreset(draft.visualInstructions, null) })}>不使用预设</Button>
            </div>
          </FieldSet>
          <div className="grid gap-4 bg-[var(--bg-subtle)] p-4 sm:grid-cols-2">
            ...
            <FieldLabel className="flex w-full cursor-pointer items-start gap-3">
              ...
              <span className="pt-2"><strong className="block text-sm font-medium">联网查证</strong><span className="mt-1 block text-sm leading-6 text-[var(--fg-secondary)]">{draft.webEnabled ? "生成方案时搜索并冻结可核验来源。" : "仅使用当前输入和参考资料。"}</span></span>
            </FieldLabel>
          </div>
